...
def main(engine_path, image_path, image_size):
    with get_engine(engine_path) as engine, engine.create_execution_context() as context:
        ...
        for i in range(5):  # This 'for' loop is for speed check
                            # Because the first iteration is usually longer
            boxes = detect(context, buffers, image_src, image_size, num_classes)

        print("Measure FPS 5 times")
        total = 0
        for _ in range(5):
            ta = time.time()

            for i in range(10):  # This 'for' loop is for speed check
                                # Because the first iteration is usually longer
                boxes = detect(context, buffers, image_src, image_size, num_classes)
...
def detect(context, buffers, image_src, image_size, num_classes):
    ...
    boxes = post_processing(img_in, 0.4, 0.6, trt_outputs, verbose=False)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        ...
        image_size = (int(sys.argv[2]), int(sys.argv[2]))
    else:
