...
class Yolo_loss(nn.Module):
    def __init__(self, n_classes=80, n_anchors=3, device=None, batch=2, image_size=608):
        ...
        for i in range(len(self.strides)):
            ...
            ref_anchors = torch.from_numpy(ref_anchors).to(self.device)
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', ncols=50) as pbar:
            ...
            for i, batch in enumerate(train_loader):
                ...
                if global_step % 10*config.subdivisions == 0:
                    ...
                    detect.detect_img(model, img_path, savename='prediction_sample.jpg', img_size=640)
...
def get_args(**kwargs):
    ...
    parser.add_argument('-g', '--gpu', metavar='G', type=str, default='-1',
                        help='GPU', dest='gpu')
    parser.add_argument('-dir', '--data-dir', type=str, default='/workspace/GitHub/YOLO/coco/images/train/',
                        help='dataset dir', dest='dataset_dir')
    parser.add_argument('-pretrained', type=str, default=None, help='pretrained yolov4.conv.137')
    # parser.add_argument('-weights', '--weights', type=str, default='/workspace/GitHub/YOLO/weights/yolov4-tiny.conv.29', help='/path/to/yolov4-tiny.weights')
    parser.add_argument('-weights', '--weights', type=str, default='/workspace/GitHub/YOLO/weights/yolov4-tiny.conv.29', help='/path/to/yolov4-tiny.weights')
...
if __name__ == "__main__":
    ...
    if cfg.use_darknet_cfg:
        ...
        if cfg.weights:
            print('-'*50)
            print('-'*50)
            print('LOADING Dakrnet with parsing cfg')
            print('load : ', cfg.weights)
            model.load_weights(cfg.weights)
            print(model.print_network())
            if cfg.pretrained:
                print('Transfer Pretrained : ', cfg.pretrained)
        ...
        # load darknet weight except yolov4 head
        weights_pth = '/workspace/GitHub/YOLO/weights/yolov4.pth'
        state_dict = torch.load(weights_pth)
        stkeys = state_dict.copy().keys()
        for k in stkeys:
            if 'head' in k:
                state_dict.pop(k)
